...
public class AnimalMove :IComponentData, IUpdate, IDisposable
{
    ...
    void DoMove()
    {
        ...
        MoveResult result = ResolveTurn(StageState, _entityState.Id, _movementRule);
    ...
    public MoveResult ResolveTurn(StageState state, int entityId, MovementRule movementRule)
        {
            if (!state.TryGetEntity(entityId, out EntityState entity) || !entity.IsAlive)
                return MoveResult.Blocked(entityId, new GridPos(0, 0), new GridPos(0, 0), MoveBlockReason.DeadEntity);

            PatrolData patrol = entity.Get<PatrolData>();
            if (patrol == null || !patrol.HasWaypoints)
                return MoveResult.Blocked(entityId, entity.Position, entity.Position, MoveBlockReason.InvalidDirection);
            ...
            MoveResult result = movementRule.TryMove(state, entityId, dir);

            if (result.Succeeded)
            {
                state.SetFacing(entityId, dir);
                state.MoveEntity(entityId, result.To);
            ...
            MoveResult rev = movementRule.TryMove(state, entityId, reverseDir);
            if (rev.Succeeded)
            {
                state.SetFacing(entityId, reverseDir);
                state.MoveEntity(entityId, rev.To);
